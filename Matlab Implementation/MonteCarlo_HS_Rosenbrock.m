...
%
%  Bu betik HS.m fonksiyonunu N_runs kez bagimsiz olarak calistirir.
...
%  GEREKLI: HS.m ayni klasorde bulunmalidir.
% =====================================================================
...
% --- HS parametreleri (varsayilan degerler) ---
HMS      = 20;     % Harmony Memory Size
r_accept = 0.85;   % Kabul orani
r_pa     = 0.5;    % Pitch Adjusting Rate
bw       = 0.15;   % Bandwidth
max_iter = 5000;   % Maksimum iterasyon

% --- Monte Carlo dongusu ---
fprintf('=== Monte Carlo: HS / Logaritmik Rosenbrock ===\n');
...
for k = 1:N_runs
    ...
    [~, bf, bp] = evalc('HarmonySearch(obj_func, lower_bound, upper_bound, num_vars, HMS, r_accept, r_pa, bw, max_iter)');
    fits(k) = bf;
    positions(k, :) = bp;
end
...
% --- Sonuc raporu ---
fprintf('\n--------------------------------------------------\n');
fprintf(' SONUCLAR (HS / Logaritmik Rosenbrock, N = %d)\n', N_runs);
fprintf('--------------------------------------------------\n');
...
figure('Name', 'Monte Carlo - HS / Logaritmik Rosenbrock', 'Color', 'w', ...
       'Position', [200, 150, 800, 550]);
histogram(fits, 'FaceColor', [0.6 0.3 0.8], 'EdgeColor', 'w');
hold on; grid on;
yl = ylim;
...
title(sprintf('HS / Logaritmik Rosenbrock  |  ort=%.4g, std=%.4g  (N=%d)', mean_fit, std_fit, N_runs));
